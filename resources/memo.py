...
class MemoListResource(Resource) :  
    @jwt_required() # 메모 생성은 로그인 한 사람만
    def post(self) : # 메모 생성
        
       # 1. 클라이언트로부터 데이터 받아온다 (post man 에서 해보기)
       #     {
       # "title": "오전 미팅 주제",
       # "date": "2022-01-22 11:00",
       # "content": "어댑터 만들어서 어쩌고 저쩌고 동화물과 백두산이 어쩌고"
       #      }
    ...
    @jwt_required() # 메모 가져오기는 로그인 한 사람만
    def get(self) :

        # 1. 클라이언트로부터 데이터를 받아온다.
        # request.args는 딕셔너리이다.
        # offset = request.arg['offset']으로도 활용가능
        # offset =request.args.get('offset')으로도 활용가능
